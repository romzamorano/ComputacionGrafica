...
GLuint VAO, VBO, shader; // u de unsigned
float color1 = 0.0f;
float color2 = 0.0f;
float color3 = 0.0f;
...
//Vertex Shader
//Lenguaje de programación para el GPU
...
void CrearTriangulo()
{
	...
	glBindVertexArray(VAO);//asignar VAO Vertex Array Object
...
void AddShader(GLuint theProgram, const char* shaderCode, GLenum shaderType) //Función para agregar los shaders a la tarjeta gráfica (no se cambia nada)
...
void CompileShaders() {
	...
	//AQUI EMPIEZAN NUEVAS LÍNEAS DE CÓDIGO
...
int main()
{
	...
	//Usando el reloj del sistema para asegurar que los colores sean random
	srand(time(NULL));

	//Se crea el primer color aleatorio antes de entrar al ciclo (así no inicia en color negro)
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		{
			//Se generan valores aleatorios entre 0.0f y 1.0f para cada color
			color1 = (float)rand() / RAND_MAX;
			color2 = (float)rand() / RAND_MAX;
			color3 = (float)rand() / RAND_MAX;
		...
		glUseProgram(shader); //Cambiar el shader activo para el bloque de código a continuación
